...
static	const char	xconsole[]	= "/dev/xconsole";
static	int		console_fd = -1;

/*
 ****************************************************************
 *	Habilita o acesso às portas				*
 ****************************************************************
 */
void
xf86EnableIO (void)
{
	if (pcntl (P_ENABLE_USER_IO) < 0)
		xf86FatalError ("Erro no pcntl (%s)\n", strerror (errno));

}	/* end xf86EnableIO */

/*
 ****************************************************************
 *	Desabilita o acesso às portas				*
 ****************************************************************
 */
void
xf86DisableIO (void)
{
	if (pcntl (P_DISABLE_USER_IO) < 0)
		xf86FatalError ("Erro no pcntl (%s)\n", strerror (errno));
...
xf86OpenConsole (void)
{
	...
	/*
	 *	A "xconsole" já foi aberta em LockServer.
	 */
	xf86Info.vtno = 0;
	xf86Info.consType = SYSCONS;

	if (xf86Verbose)
		ErrorF ("\nUsando o dispositivo \"%s\" (fd = %d)\n\n", xconsole, console_fd);

	xf86Info.consoleFd = xf86Info.screenFd = console_fd;
...
}	/* end xf86CloseConsole */

/*
 ****************************************************************
 *	Abre a console, servindo de tranca para o servidor	*
 ****************************************************************
 */
void
LockServer (void)
{
	if ((console_fd = open (xconsole, O_RDONLY|O_NONBLOCK)) < 0)
	{
		if (errno == EBUSY)
			FatalError ("Já existe um servidor X rodando\n");

		FatalError ("Não pude abrir \"%s\"\n", xconsole);
	}
...
xf86SoundKbdBell (int loudness, int pitch, int duration)
{
    	if (loudness && pitch)
	{
		ioctl (xf86Info.consoleFd, XC_BEEP, pitch);
#if (0)
		ioctl
		(	xf86Info.consoleFd, KDMKTONE,
...
/*
 ****************************************************************
 *	Altera os "leds" do teclado				*
 ****************************************************************
 */
void
xf86SetKbdLeds (int leds)
{
	ioctl (xf86Info.consoleFd, XC_SETLEDS, leds);

}	/* end xf86SetKbdLeds */

/*
 ****************************************************************
 *	Obtém os "leds" do teclado				*
 ****************************************************************
 */
int
xf86GetKbdLeds (void)
...
#if (0)
	ioctl (xf86Info.consoleFd, XC_GETLEDS, &leds);
#endif
	return (leds);
...
/*
 ****************************************************************
 *	Modifica a taxa de repetição do teclado			*
 ****************************************************************
 */
void
 ...
 ****************************************************************
 */
static TERMIO	kbdtty;

void
xf86KbdInit (void)
{
	ioctl (xf86Info.consoleFd, TCGETS, &kbdtty);
}

 ...
 *			video.c					*
 *								*
 *	Mapeamento da Memória de Vídeo				*
 *								*
 *	Versão	1.0.0, de 02.01.98				*
 *								*
 *	Módulo: hw/os/video.c					*
 *		X Windows - Servidor				*
...
/*
 ****************************************************************
 *	Verifica se pode usar o "/dev/mem" (NÃO pode)		*
 ****************************************************************
 */
static void
...
mapVidMem (int ScreenNum, unsigned long Base, unsigned long Size, int flags)
{
	...
	if ((int)(vp = phys ((void *)Base, Size, O_RDWR|O_PHYS)) == -1)
	{
		FatalError
		(	"Erro no phys: addr = %P, size = %d\n",
			Base, Size
		);
	}
	else
 ...
 */
int
xf86ReadBIOS (ulong Base, ulong Offset, uchar *Buf, int Len)
{
	const void	*vp;

	vp = phys ((void *)Base + Offset, Len, O_PHYS|O_RDONLY);

	if ((int)vp == -1)
	{
		ErrorF
		(	"xf86ReadBIOS: erro no \"phys\" (%s)\n",
			strerror (errno)
		);
...
xf86ReadBIOS (ulong Base, ulong Offset, uchar *Buf, int Len)
{
	...
	{
		ErrorF
		(	"xf86ReadBIOS: não pude abrir \"%s\" (%s)\n",
			DEV_MEM, strerror (errno)
		);
		return (-1);
	}
	...
	{
		ErrorF
		(	"xf86ReadBIOS: erro no lseek em \"%s\" (%s)\n",
			DEV_MEM, strerror(errno)
		);
		close (fd);
		return (-1);
	...
	{
		ErrorF
		(	"xf86ReadBIOS: erro no read em \"%s\" (%s): addr = %P, nb = %d\n",
			DEV_MEM, strerror(errno), Buf, Len
		);
		close (fd);
		return (-1);
	}

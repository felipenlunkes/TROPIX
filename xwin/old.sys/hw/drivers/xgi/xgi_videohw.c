/* $XFree86: xc/programs/Xserver/hw/xfree86/drivers/xgi/xgi_videohw.c,v 1.1 2005/05/02 13:28:02 dawes Exp $ */
...
staticvoid _XGIWrite(XGIPtr pXGI, CARD32 reg, CARD32 data)
{
...
void XGI_SetSRRegMask(XGIPtr pXGI, CARD8 reg, CARD8 data, CARD8 mask)
{
    CARD8   old;
...
/* ----------------T05_EnableCapture()---------------- */
#ifdef CAPTURE_340A1
static void SetEnableCaptureReg(XGIPtr pXGI, Bool bEnable, Bool bFlip)
{
   if (bEnable)
   {
...
XGISetXvGamma(XGIPtr pXGI)
{
	 ...
	 */
    XGI_SetSRRegMask(pXGI, Index_SR_Power_Management, 0x08, 0x18);
    for(i = 0; i <= 255; i++) {
    ...
    XGI_SetSRRegMask(pXGI, Index_SR_Power_Management, backup, 0xff);
}
...
/* ----------------SetDDAReg()------------------------------------ */
static VOID SetDDAReg (XGIPtr pXGI, float scale)
{
...
XGIResetVideo(ScrnInfoPtr pScrn)
{
    ...
    XGI_SetSRRegMask(pXGI, Index_SR_CRT_Misc_Ctrl,    0x00,  0x02);
    ...
    XGI_SetSRRegMask(pXGI, Index_Video_Process,        0x00, 0x02);
...
void
XGI_SetColorkeyReg(XGIPtr pXGI, CARD32 colorkey)
{
    CARD8 r, g, b;
...
void
XGI_SetVideoBrightnessReg(XGIPtr pXGI, INT32 value)
{
    CARD8   brightness;
...
void
XGI_SetVideoContrastReg(XGIPtr pXGI, INT32 value)
{
    CARD8   contrast;
...
void
XGI_SetVideoHueReg(XGIPtr pXGI, INT32 value)
{
    CARD8   hue;
...
void
XGI_SetVideoSaturationReg(XGIPtr pXGI, INT32 value)
{
    CARD8   saturation;
...
void
XGI_SetOverlayReg(XGIPtr pXGI, XGIOverlayPtr pOverlay)
{
...
void
XGI_SetCloseOverlayReg(XGIPtr pXGI)
{
...
void
XGI_SetSelectOverlayReg(XGIPtr pXGI, CARD8 index)
{
  SetVideoRegMask(pXGI, Index_VI_Control_Misc2, index, 0x01);
}

void
XGI_SetEnableOverlayReg(XGIPtr pXGI, Bool bEnable)
{
        if (bEnable)
...
void
XGI_EnableCaptureAutoFlip(XGIPtr pXGI, Bool bEnable)
{
   if (bEnable)
      SetVideoRegMask(pXGI, Index_VI_Control_Misc0, 0x01, 0x01);
   else

...
FVWMWIN *FocusOnNextTimeStamp = NULL;
 ...
 *
 ****************************************************************************/
void Destroy(FVWMWIN *Tmp_win)
{ 
  int i;
  extern FVWMWIN *ButtonWindow;
...
/*  XUnmapWindow(dpy, Tmp_win->frame); */
  ...
  Broadcast(M_DESTROY_WINDOW,3,Tmp_win->w,Tmp_win->frame,(ulong)Tmp_win);
 ...
 *
 ************************************************************************/
void RestoreWithdrawnLocation (FVWMWIN *tmp,Bool restart)
{
  int a,b,w2,h2;
...
void RaiseWindow(FVWMWIN *t)
{
  ...
  count = 1;
  Broadcast(M_RAISE_WINDOW,3,t->w,t->frame,(ulong)t);
  ...
  for (t2 = Scr.FvwmRoot.next; t2 != NULL; t2 = t2->next)
	...
	{
	  count++;
	  Broadcast(M_RAISE_WINDOW,3,t2->w,t2->frame,(ulong) t2);
...
void LowerWindow(FVWMWIN *t)
{
  XLowerWindow(dpy,t->frame);

  Broadcast(M_LOWER_WINDOW,3,t->w,t->frame,(ulong)t);
  ...
  Scr.LastWindowRaised = (FVWMWIN *)0;
}
...
*/
void
fvwm_msg (int type, char *id, char *msg,...)
{
	char		 *typestr;
	va_list		args;
	...
	fprintf (stderr,"[%s] %s (%s): %s ", fulltime (), pgname, id, typestr);

...
InitEventHandlerJumpTable (void)
{
	...
	for (i = 0; i < LASTEvent; i++)
	{
		...
		event_name[i] = "Evento não tratado";
	}
...
HandleEnterNotify (void)
{
	...
	/*
	 *	EnterNotify em um dos PanFrames: ativa a paginação.
	 */
	if
...
#if (0)	/****************************************************/
msg ("Janela %s %s", Tmp_win->name, (Tmp_win->flags & VISIBLE) ? "visível" :
"invisível");
#endif	/****************************************************/
...
open_grave_device (void)
{
	if ((grave_fd = open (grave_dev, O_RDONLY)) < 0)
		msg ("$Não consegui abrir o \"%s\"", grave_dev);

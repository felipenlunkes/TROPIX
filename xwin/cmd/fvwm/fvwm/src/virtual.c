 ...
 *			src/virtual.c				*
 *								*
 *	Processa a paginação da tela virtual			*
 *								*
...
/*
 ****************************************************************
 *	Verifica se o mouse encostou na borda, paginando	*
 ****************************************************************
 */
void
		...
		int *delta_x, int *delta_y, Bool Grab)
{
	...
	*delta_y = 0;
	...
	while (total < Scr.ScrollResistance)
	{
		...
		if
		(	XCheckWindowEvent
			(	dpy, Scr.PanFrameTop.win,
				LeaveWindowMask, &Event
			) ||
			XCheckWindowEvent
			(	dpy, Scr.PanFrameBottom.win,
				LeaveWindowMask, &Event
			) ||
			XCheckWindowEvent
			(	dpy, Scr.PanFrameLeft.win,
				LeaveWindowMask, &Event
			) ||
			XCheckWindowEvent
...
/*
 ****************************************************************
 *	Modifica o estado dos PanFrames				*
 ****************************************************************
 *
...
checkPanFrames (void)
{
	...
	if ((Scr.flags & WindowsCaptured) == 0)
		return;
...
/*
 ****************************************************************
 *	Garante que os PanFrames estão em cima de tudo		*
 ****************************************************************
 */
void
raisePanFrames (void)
 ...
 ****************************************************************
 */
#define	pan_create(x,y,dx,dy) 	XCreateWindow				\
				(	dpy, Scr.Root, 			\
					x, y, dx, dy,			\
					0,	/* sem borda */		\
...
initPanFrames (void)
{
	...
	Scr.PanFrameTop.win = 	pan_create
				(	0, 0, 
					Scr.MyDisplayWidth,
					PAN_FRAME_THICKNESS
				);
	...
	Scr.PanFrameLeft.win = 	pan_create
	...
	Scr.PanFrameRight.win = pan_create
	...
	Scr.PanFrameBottom.win = pan_create
				 ( 	0,
					Scr.MyDisplayHeight - PAN_FRAME_THICKNESS,
					Scr.MyDisplayWidth,
					PAN_FRAME_THICKNESS
				 );
...
/*
 ****************************************************************
 *	Move o ViewPort dentro da tela virtual			*
 ****************************************************************
 */
void
	...
	if (deltax != 0 || deltay != 0)
	{
		for (t = Scr.FvwmRoot.next; t != NULL; t = t->next)
		{
			...
			)
			{
				...
				SetupFrame
				(	t, t->frame_x+ deltax, t->frame_y + deltay,
		...
		for (t = Scr.FvwmRoot.next; t != NULL; t = t->next)
		{
			 ...
			 */
			if
			(	((t->flags & STICKY) || (t->flags & StickyIcon)) &&
				(t->flags & ICONIFIED) &&
				((t->flags & ICON_MOVED) == 0) &&
				((t->flags & ICON_UNMAPPED) == 0)
			)
				AutoPlace (t);
		}
	}
...
/*
 ****************************************************************
 *	Move para um novo desktop				*
 ****************************************************************
 */
void
changeDesks_func (XEvent *eventp, Window w, FVWMWIN *tmp_win,
...
}	/* end changeDesks_func */

void
changeDesks (int val1, int val2)
{
	int oldDesk;
	FVWMWIN *FocusWin = 0, *t;
	static FVWMWIN *StickyWin = 0;
	...
	/*
	 *	OK, someone beat me up, but I don't like this.
	 *	If you are a predominantly
	 *	focus-follows-mouse person, but put in one sticky
	 *	click-to-focus window (typically because you don't really
	 *	want to give focus to this window),
 ...
 *************************************************************************/
void
changeWindowsDesk (XEvent *eventp, Window w, FVWMWIN *t,
			ulong context, char *action, int *Module)
{
	long	val1;
	int	val1_unit, n;
	 ...
	 */
	if
	(	!((t->flags & ICONIFIED) && (t->flags & StickyIcon)) &&
		((t->flags & STICKY) == 0) && ((t->flags & ICON_UNMAPPED) == 0)
	)
	{
...
}	/* end changeWindowsDesk */

void
scroll (XEvent *eventp, Window w, FVWMWIN *tmp_win, ulong context,
...
void
goto_page_func (XEvent *eventp, Window w, FVWMWIN *tmp_win,
		    ulong context, char *action, int *Module)
{
	...
	if (val1_unit != Scr.MyDisplayWidth || val2_unit != Scr.MyDisplayHeight)
	{

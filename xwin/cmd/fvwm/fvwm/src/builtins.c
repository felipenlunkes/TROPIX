 ...
 *			src/builtins.c				*
 *								*
 *	Processa os comandos intrínsecos			*
 *								*
 ...
 *      tmp_win - pointer to FVWMWIN Structure to patch up
 ...
 *
 ***********************************************************************/
int DeferExecution(XEvent *eventp, Window *w,FVWMWIN **tmp_win,
		   ulong *context, int cursor, int FinishEvent)
...
void WarpOn(FVWMWIN *t,int warp_x, int x_unit, int warp_y, int y_unit)
{
  ...
  if(t == (FVWMWIN *)0 || (t->flags & ICONIFIED && t->icon_w == None))
    return;
...
void Bell(XEvent *eventp,Window w,FVWMWIN *tmp_win,ulong context,
...
void movecursor(XEvent *eventp,Window w,FVWMWIN *tmp_win,ulong context,
		char *action, int *Module)
{
		   ...
		   ulong context, char *action, int *Module)
{
  ...
  if(val1 != 0)
  {
    FocusOn((FVWMWIN *)val1,0);
    if (((FVWMWIN *)(val1))->flags & ICONIFIED)
    {
      DeIconify((FVWMWIN *)val1);
      FocusOn((FVWMWIN *)val1,0);
    }
  }
}
                 ...
                 ulong context, char *action,int* Module)
{
  ...
  if(Scr.VyMax <0)
    Scr.VyMax = 0;
  Broadcast(M_NEW_PAGE,5,Scr.Vx,Scr.Vy,Scr.CurrentDesk,Scr.VxMax,Scr.VyMax);
                   ...
                   ulong context, char *action,int* Module)
{
  ...
  while(parm && parm[0]!='\0')
  {
    ...
    {
      Scr.TitleStyle.justify = JUSTIFY_LEFT;
    }
    else if (strncasecmp(parm,"rightjustified",14)==0)
    {
      Scr.TitleStyle.justify = JUSTIFY_RIGHT;
    }
    else if (strncasecmp(parm,"raised",6)==0)
    {
      Scr.TitleStyle.appearance = TITLE_RAISED;
    }
    else if (strncasecmp(parm,"sunk",4)==0)
    {
      Scr.TitleStyle.appearance = TITLE_SUNK;
    }
    else if (strncasecmp(parm,"flat",4)==0)
    {
      Scr.TitleStyle.appearance = TITLE_FLAT;
    }
    else if (strncasecmp(parm,"mwm",3)==0)
    {
...
void LoadWindowFont(XEvent *eventp,Window win,FVWMWIN *tmp_win,
                    ulong context, char *action,int* Module)
{
  char *font;
  FVWMWIN *tmp,*hi;
...
void FreePixmapButtonData(Display *dpy, PixmapButtonData *pbd)
{
  if (pbd->up)
    DestroyPicture(dpy, pbd->up);
  pbd->up=NULL;
  if (pbd->down)
    DestroyPicture(dpy, pbd->down);
  pbd->down=NULL;
}
 ...
 *
 ****************************************************************************/
void ButtonStyle(XEvent *eventp,Window junk,FVWMWIN *tmp_win,
                 ulong context, char *action,int* Module)
{
  int button,n,num;
  char * letter=action;
  char * text=action;
  char button_style[256];
  int offset,i;
  int num_coords;
  int *x_coords;
  int *y_coords;
  ...
  Scr.button_style[button] = NormalButton;

  FreePixmapButtonData(dpy,&Scr.pixmap_button[button]);

  text=offset+letter;  

  /* get button style (Simple,Normal,Pixmap) */
  num = sscanf(text, "%s%n", button_style, &offset);

  if (num != 1 || button_style == NULL)
  {
    fvwm_msg(ERR,"ButtonStyle","Bad button style (1a) in line %s",action);
    return;
  }  

  if (!(strlen(button_style)<=2 && isdigit(*button_style)))
  {
    text+=offset;
  }
  ...
  if (strncasecmp(button_style,"Simple",6)==0)
  {
    Scr.button_style[button] = SimpleButton;
  }
  else if (strncasecmp(button_style,"Normal",6)==0 || 
	   (strlen(button_style)<=2 && isdigit(*button_style)))
  {
    ...
    for(i=0;i<num_coords;i++)
    {
      ...
      if(num != 3)
      {
	fvwm_msg(ERR,"ButtonStyle","Bad button style (3) in line %s",action);
	return;
      }
    }
  }
  else if (strncasecmp(button_style,"Pixmap",6)==0 || strncasecmp(button_style,"FullPixmap",10)==0)
  {
    ...
    num = sscanf(text, "%s%s", file1, file2);  
    
    if (num > 0)
    {
      /* get "up" position pixmap */
      Scr.pixmap_button[button].up = CachePicture(dpy,
                                                  Scr.Root,
                                                  IconPath,
                                                  PixmapPath,
                                                  file1);
      if (Scr.pixmap_button[button].up == NULL)
      {
	fvwm_msg(ERR,"ButtonStyle","Couldn't load up pixmap button %s: %s", 
		 file1, action);
	okay=False;
      }
    } 
    ...
    {
      fvwm_msg(ERR,"ButtonStyle","No pixmap given: %s", action);
      okay=False;
    }
    
    if (num > 1)
    {
      /* get "down" position pixmap */
      Scr.pixmap_button[button].down = CachePicture(dpy,
                                                    Scr.Root,
                                                    IconPath,
                                                    PixmapPath,
                                                    file2);
      if (Scr.pixmap_button[button].down == NULL)
      {
	fvwm_msg(ERR,"ButtonStyle","Couldn't load down pixmap button %s: %s", 
		 file2, action);
	okay=False;
      }
    } 
    ...
    {
      /* set "down" pixmap to same value as "up" pixmap if only 1 specified */
      Scr.pixmap_button[button].down=Scr.pixmap_button[button].up;
    }
    
    if (okay) 
    {
      if (strncasecmp(button_style,"FullPixmap",10)==0)
	  Scr.button_style[button] = FullPixmapButton;
      else
	  Scr.button_style[button] = PixmapButton;
    }
    else
    {
      fvwm_msg(ERR,"ButtonStyle",
               "Failure creating Pixmap button face, reverting to Simple: %s",
	       action);
      Scr.button_style[button] = SimpleButton;	
    }
  } 
  ...
  {
    fvwm_msg(ERR,"ButtonStyle",
             "Bad button style %s in line %s",button_style,action);
    return;
  } 
 ...
 *
 **************************************************************************/
FVWMWIN *Circulate(char *action, int Direction, char **restofline)
{
  int l,pass = 0;
  FVWMWIN *fw, *found = NULL;
...
void PrevFunc(XEvent *eventp,Window junk,FVWMWIN *tmp_win,
              ulong context, char *action,int* Module)
{
  FVWMWIN *found;
  char *restofline;
...
void NextFunc(XEvent *eventp,Window junk,FVWMWIN *tmp_win,
              ulong context, char *action,int* Module)
{
  FVWMWIN *found;
  char *restofline;
...
void NoneFunc(XEvent *eventp,Window junk,FVWMWIN *tmp_win,
              ulong context, char *action,int* Module)
{
  FVWMWIN *found;
  char *restofline;
...
void Recapture(XEvent *eventp,Window junk,FVWMWIN *tmp_win,
               ulong context, char *action,int* Module)
{
  FVWMWIN *tmp,*next;		/* temp fvwm window structure */
  Window w;

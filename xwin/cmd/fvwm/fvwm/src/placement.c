 ...
 *			src/placement.c				*
 *								*
 *	Gerencia o posicionamento de janelas			*
 *								*
 *	Versão	3.0.0, de 29.07.97				*
 *		4.4.0, de 20.03.03				*
 *								*
 *	Módulo: cmd/fvwm					*
 *		X Windows - Clientes				*
 ...
 */
void
SmartPlacement (FVWMWIN *t, int width, int height, int *x, int *y)
{
	int		temp_h,temp_w;
	...
	while (!loc_ok && test_y + temp_h < Scr.MyDisplayHeight)
	{
		test_x = 0;
		while (!loc_ok && test_x + temp_w < Scr.MyDisplayWidth)
		{
			loc_ok = True;
			test_window = Scr.FvwmRoot.next;

			while (loc_ok && test_window != (FVWMWIN *)0)
			{	
				if (test_window->Desk == Scr.CurrentDesk)
				{
					if
					(	!(test_window->flags & ICONIFIED) &&
						test_window != t
					)
					{
 ...
 */
Bool
PlaceWindow (FVWMWIN *tmp_win, ulong tflag, int Desk)
{
	FVWMWIN	*t;
	...
	else
	{
		...
		if
		(	(tmp_win->wmhints) &&
			(tmp_win->wmhints->flags & WindowGroupHint) &&
		...
		)
		{
			...
			{
				if
				(	t->w == tmp_win->wmhints->window_group ||
					(	t->wmhints &&
		...
		if
		(	XGetWindowProperty
			(	dpy, tmp_win->w, _XA_WM_DESKTOP, 0L, 1L, True,
   ...
   */
	...
	)
	{
		...
		if (tflag & RANDOM_PLACE_FLAG)
		{
			...
			if
			(	tmp_win->frame_y + 2 * tmp_win->boundary_width
 ...
 ****************************************************************
 */
static int	xoffset[] = {0, -1,  0,  1, -1, 0, 1, -1, 0, 1, 0};
static int	yoffset[] = {0, -1, -1, -1,  0, 0, 0,  1, 1, 1, 0};

void
GetGravityOffsets (FVWMWIN *tmp, int *xp, int *yp)
{
	int	g;

	g = tmp->hints.flags & PWinGravity

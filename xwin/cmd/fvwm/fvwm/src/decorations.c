 ...
 *			src/decorations.c			*
 *								*
 *	Processa os enfeites das janelas			*
 *								*
...
void GetMwmHints(FVWMWIN *t)
{
  ...
  if(XGetWindowProperty (dpy, t->w, _XA_MwmAtom, 0L, 20L, False,
			 ...
			 &bytesafter,(uchar **)&t->mwm_hints)==Success)
    {
...
void GetOlHints(FVWMWIN *t)
{
  ...
  if (XGetWindowProperty (dpy, t->w, _XA_OL_WIN_ATTR, 0L, 20L, False,
                          ...
                          &bytesafter,(uchar **)&hints)==Success)
    {
      if (nitems > 0)
        {
          if (nitems == 3)
 ...
 *
 *****************************************************************************/
void SelectDecor(FVWMWIN *t, ulong tflags, int border_width, 
		 int resize_width)
{
  int decor,i;
  PropMwmHints *prop;

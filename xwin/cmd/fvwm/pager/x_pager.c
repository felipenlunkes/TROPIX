...
Pixel GetColor(char *name)
{
  ...
  color.pixel = 0;
   if (!XParseColor (dpy, attributes.colormap, name, &color)) 
	msg ("\"%s\": nome de cor inválido", name);
   else if(!XAllocColor (dpy, attributes.colormap, &color)) 
	msg ("erro ao alocar a cor \"%s\"", name);
  return color.pixel;
}
	  ...
	  for(i=0;i<ndesks;i++)
	    {
	      if(Event->xany.window == Desks[i].w)
		{
		  XQueryPointer(dpy, Desks[i].w, &_JunkRoot, &_JunkChild,
				&_JunkX, &_JunkY,&x, &y, &_JunkMask);
		  Scroll(i,x,y);
		}
	    }
	  if(Event->xany.window == icon_win)
	    {
	      XQueryPointer(dpy, icon_win, &_JunkRoot, &_JunkChild,
			    &_JunkX, &_JunkY,&x, &y, &_JunkMask);
	      IconScroll(x,y);
	    }
	}
...
void MovePage(void)
{
  ...
  if(Scr.CurrentDesk != icon_desk_shown)
	...
	{
	  msg ("cannot allocate window name");
	  return;
	}
 ...
 *
 ************************************************************************/
XErrorHandler FvwmErrorHandler(Display *dp, XErrorEvent *event)
{
#if 0
  Window root;
  unsigned border_width, depth;
  ...
  if(XGetGeometry(dp,Scr.Pager_w,&root,&x,&y,
		  (unsigned *)&window_w,(unsigned *)&window_h,

 ...
 *			FvwmPager.c				*
 *								*
 *	Mostra a sinopse dos DeskTops usados no "fvwm"		*
 *								*
 *	Versão	3.0.0, de 22.07.97				*
 *		3.1.6, de 29.01.99				*
 *								*
 *	Módulo: programs/fvwm/modules/FvwmPager			*
 *		X Windows - Clientes				*
 ...
 ****************************************************************
 */
entry	const char	pgversion[]	= "Versão 3.1.6, de 29.01.99";
...
main (int argc, char **argv)
{
  ...
  strcpy(MyName, temp);
	...
	if (argc != 8)
		msg ("$Este programa deve ser executado via \"fvwm\"");
...
fprintf(stderr,"[main]: Desks[%d].Dcolor == %s\n",i,Desks[i].Dcolor);
	...
	if(Scr.Root == None) 
		msg ("$Tela %d inválida", Scr.screen);
...
/*
 ****************************************************************
 *	Examina e processa as mensagens recebidas		*
 ****************************************************************
 */
void
process_message (ulong type, ulong *body)
{
	switch(type)
	{
	    case M_ADD_WINDOW:
	    ...
	    case M_FVWM_IS_DYING:
		msg ("O \"fvwm\" notificou sua saída do ar");
 ...
 */
int
My_XNextEvent (Display *dp, XEvent *event)
{
	ulong		header[HEADER_SIZE];
	      ...
	      (strncasecmp(tline,CatString3("*", MyName, "Back"),Clength+5)==0))
	{
	  if(Scr.d_depth > 1)
	    ...
	    for (n=0;n<ndesks;n++)
	      ...
	      (strncasecmp(tline,CatString3("*",MyName,"DeskColor"),Clength+10)==0))
	{
	  desk = desk1;
...
/*
 ****************************************************************
 *	Fecha a conexão com o servidor antes de terminar	*
 ****************************************************************
 */
void
close_all (void)

...
main (int argc, register char *argv[])
{
	 ...
	 *	Abre o "endpoint" RAW
	 */
	if ((fd = t_open ("/dev/itnraw", O_RDWR, (T_INFO *)NULL)) < 0)
	{
	...
	/*
	 *	Obtém as informações RAW
	 */
	if (t_getinfo (fd, &info) < 0)
	{
		fprintf (stderr, "RAW: Não consegui obter as informações\n");
		perror (pgname);
		exit (1);
	}

	/*
	 *	Imprime as informações RAW
	 */
	printf
	(	"\nRAW:\n\n"
		"info.addr = %d\n"
	...
	if (t_getinfo (fd, &info) < 0)
	{
		fprintf (stderr, "UDP: Não consegui obter as informações\n");
		perror (pgname);
		exit (1);
	}
	...
	if (t_getinfo (fd, &info) < 0)
	{
		fprintf (stderr, "TCP: Não consegui obter as informações\n");
		perror (pgname);
		exit (1);
	}

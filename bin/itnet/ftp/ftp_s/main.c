 ...
 ****************************************************************
 */
#define	CNTL_TIME_OUT	600	/* Tempo de espera por um comando */
...
entry int	type = TYPE_A;	/* Tipo corrente */
entry int	form = FORM_N;	/* Forma corrente */
entry int	stru = STRU_F;	/* Tipo corrente */
entry int	mode = MODE_S;	/* Modo corrente */

entry int	cntl_fd;	/* Conexão de controle */

entry FILE	*cntl_fp;	/* Conexão de controle */

entry INADDR	server_cntl_addr[2];	/* Endereço do servidor */

entry char	client_node_nm[80];	/* Nome do cliente */

entry int	snd_dis;	/* Em caso de erro grave envia DIS */
...
main (int argc, register const char *argv[])
{
	...
	if ((cntl_fp = fdopen (cntl_fd, "w")) == NOFILE)
		error ("$*Não consegui obter um FILE para \"%s\"", tcp_dev_nm);

	/*
	 *	Vou realizar o "bind"
	 */
	serv_addr[0].a_port = listen_port;	/* Usa o FTP_CNTL_PORT */
	...
	if (t_bind (cntl_fd, &bind, (T_BIND *)NULL) < 0)
		error ("$*Não consegui atribuir endereço");

	/*
	 *	Desliga o "timeout" do TCP/IP
	 */
   ...
   /***	req_optmgmt.opt.len    = 0; ***/
	...
	if (t_optmgmt (cntl_fd, &req_optmgmt, &ret_optmgmt) < 0)
		error ("$*Não consegui negociar (DEFAULT)");

	req_optmgmt.flags = T_NEGOTIATE;

	options.max_silence = 0;
   ...
   /***	call.udata.len = 0; ***/
	...
	if (t_accept (listen_fd, cntl_fd, &call) < 0)
		error ("$*Erro no t_accept");

	/*
	 *	Agora não precisa mais do "listen_fd"
	 */
	client_data_addr.a_addr = client_addr.a_addr;

	t_close (listen_fd);

	if ((str = t_addr_to_node (cntl_fd, client_addr.a_addr)) == NOSTR)
		str = t_addr_to_str (client_addr.a_addr);

	strcpy (client_node_nm, str);

	/*
	 *	Obtém o endereço local do servidor
	 */
	if (t_getaddr (cntl_fd, server_cntl_addr) < 0)
		error ("$*Não consegui obter o endereço local");

	reply
	(	220, "%s (%s, %s) Servidor FTP (%s)",
	...
	for (EVER)
	{
		...
		if ((n = t_rcv (cntl_fd, comando, sizeof (comando), (int *)NULL)) < 0)
			error (NOSTR);
...
#undef	DEBUG
		...
		{
			char			line[BLSZ];
			register const char	*ip = comando;
			register char		*op = line;
			register char		c;
			register int		i;
...
check_ready_state (void)
{
	...
	reply (503, "Este comando só pode ser executado com um usuário em sessão");
...
/*
 ****************************************************************
 *	Envia uma mensagem longa para o cliente			*
 ****************************************************************
 */
void
long_reply (int code, const char *format, ...)
{
	...
	if (t_snd (cntl_fd, msg, strlen (msg), T_PUSH) < 0)
		error (NOSTR);
...
error (const char *format, ...)
{
	...
	if (log_fp != NOFILE)
	{
		...
		{
			if   (event != 0)
				fprintf (log_fp, " (%s)", t_strevent (cntl_fd, event));
			elif (save_errno != 0)

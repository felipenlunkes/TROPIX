 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão:	4.8.0, de 02.08.05";

#define	PMAP_PORT	111

/*
 ******	Converte um longo para BIG_ENDIAN (ou vice-versa) *******
 */
#define ENDIAN_CV(x)	(((x) << 24)		  |	\
			(((x) & 0x0000FF00) << 8) |	\
			(((x) & 0x00FF0000) >> 8) |	\
			(((unsigned)(x) >> 24)))

/*
 ******	Definições do protocoloes *******************************
 */
enum {	RPC_VERS = 2 };
...
/*
 ******	Estrutura para o envio de um pedido de PORTA ************
 */
typedef struct
{
	int	r_xid;		/* No. do pedido */
	int	r_mtype;	/* Pedido/resposta */
	int	r_rpcver;	/* Versão do protocolo do RPC */
	int	r_prog;		/* No. do programa do PORT MAPPER */
	int	r_vers;		/* Versão do protocolo do PORT MAPPER */
	int	r_proc;		/* Função pedida */

	int	r_cred[2];	/* Autenticação */
	int	r_verf[2];

	int	r_req_prog;	/* Dados do programa do qual desejamos  */
	int	r_req_vers;	/* saber a porta */
	int	r_req_prot;
	int	r_req_port;
...
/*
 ******	Estrutura para o recebimento de uma PORTA ***************
 */
typedef struct
{
	int	r_xid;		/* No. do pedido */
...
const int		tst = ENDIAN_CV (100000);
 ...
 */
#define N_SERVTB	10	/* No. máximo de servidores */	
...
entry SERVTB	servtb[N_SERVTB+1];	/* A tabela de servidores */	
...
send_req (void)
{
	...
	dg.r_xid	= long_endian_cv (++id);
	dg.r_mtype	= ENDIAN_CV (RPC_CALL);
	dg.r_rpcver	= ENDIAN_CV (RPC_VERS);
	dg.r_prog	= ENDIAN_CV (PMAP_PROG);
	dg.r_vers	= ENDIAN_CV (PMAP_VERS);
	dg.r_proc	= ENDIAN_CV (PMAPPROC_GETPORT);

   /***	dg.r_cred	= 0;	***/
   /***	dg.r_verf	= 0;	***/

	dg.r_req_prog	= ENDIAN_CV (MOUNTPROG);
	dg.r_req_vers	= ENDIAN_CV (MOUNTVERS);
	dg.r_req_prot	= ENDIAN_CV (IPPROTO_UDP);
	dg.r_req_port	= 0;	/* Ignorado */

	/*
	 *	Obtém o endereço do nó remoto
	 */
	if ((dst_addr = t_node_to_addr (udp_fd, dst_nome_nm, (long *)NULL)) == -1)
		error ("$*Não consegui obter o endereço de \"%s\"", dst_nome_nm);

	/*
	 *	Envia o datagrama UDP
	 */
	client_addr.a_addr = dst_addr;
	client_addr.a_port = PMAP_PORT;

	ud.addr.len	= sizeof (INADDR);
   	ud.addr.buf	= &client_addr;

	ud.udata.len	= sizeof (dg);
	ud.udata.buf	= &dg;

	ud.opt.len	= 0;

	if (t_sndudata (udp_fd, &ud) < 0)
		error ("$*Não consegui enviar o datagrama");

#if (0)	/*******************************************************/
	{
		int fd = creat ("dg", 0644);

		write (fd, &dg, sizeof dg);

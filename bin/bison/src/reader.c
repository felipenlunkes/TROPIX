...
{
  start_flag = 0;
  startval = (bucket *)NULL;  /* start symbol not specified yet. */
...
get_type_name (int n, symbol_list *rule)
{
  ...
  while (i < n)
    {
      rp = rp->next;
      if (rp == (symbol_list *)NULL || rp->sym == (bucket *)NULL)
	fatal(msg);
      i++;
    }
  ...
  while (t != TWO_PERCENTS && t != ENDFILE)
      ...
      if (t == IDENTIFIER || t == BAR)
	{
	  ...
	  /* If $$ is being set in default way,
	    ...
	    {
	      if (lhs->type_name == NOSTR || first_rhs->type_name == NOSTR
	  ...
	  else if (!xactions && !first_rhs && lhs->type_name != NOSTR)
	    fprintf(stderr,

...
entry unsigned	outcnt;
entry unsigned	inptr;

entry ulong	crc_32;		/* CRC usado pelo deflate/inflate */
...
read_zip (ZIPFILE *zp)
{
	int		count, cnt, offset = 0;
	char		crc[8];

	count = MIN (col_size, INBUFSIZ);
	...
	while (count > 0)
	{
		...
		bread (zp->z_base + offset, cnt);

		bread (crc, sizeof (crc) );

		if (atoi (crc) != crc16 (zp->z_base + offset, cnt) )
		{
 ...
 */
void
write_zip (int c, ZIPFILE *zp)
{
	int		count;

	count = zp->z_ptr - zp->z_base;

	write_buf (ofd, zp->z_base, count);
...
flush_window (void)
{
	...
	while (outcnt > 0)
	{
		count = MIN (outcnt, BLSZ);

		function (window + offset, count);

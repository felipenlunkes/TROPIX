 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão: 3.2.0, de 23.05.98";
...
entry int	sb_fd;		/* Descritor da placa SB16 */
...
main (int argc, const char *argv[])
{
	 ...
	 *	Abre a placa de som
	 */
	if ((sb_fd = open ("/dev/sb", O_WRONLY)) < 0)
	...
	if (argc > 0 && !dflag)
	{
		size = 32 * KBSZ; dma_size = size << 1;

		area0 = malloc (2 * dma_size) + dma_size;

		if ((paddr = pcntl (P_GET_PHYS_ADDR, area0)) == -1)
			error ("$*Não consegui obter o endereço físico da área");

		offset = paddr - (paddr & ~(dma_size - 1));

		area0 = area0 - offset; area1 = area0 + size;
	}
	...
	while ((file_nm = *argv++) != NOSTR)
	{
		 ...
		 */
		if (!dflag)
			sb_play_file (file_nm, file_fp, size, area0, area1);
 ...
 */
void
sb_play_file (const char *file_nm, FILE *file_fp, int size, char *area0,  char *area1)
{
	...
	(	line,
		...
		xwin_active ? cv_bytes_to_KB_or_MB (wave.w_size) : sb_edit_time (wave.w_size)
	);
	...
	if (ioctl (sb_fd, SB_SET_MODE, mode) < 0)
	...
	for (even_odd = 0, count = wave.w_size; count > 0; count -= cnt)
	{
		...
		if (write (sb_fd, data_ptr, size) != size)
	 ...
	 *	Escreve EOF
	 */
	if (write (sb_fd, NOSTR, size) != size)
...
#define		POINTs		5
	...
	{
		if (i == POINTs)
			cp = sb_edit_time (wave.w_size);
		else
			cp = sb_edit_time (i * wave.w_size / POINTs);
...
help (void)
{
	fprintf
	(	stderr,
		"%s - Toca música PCM através da placa de som SB-16\n"
		"\n%s\n"
		"\nSintaxe:\n"
		"\t%s [-vdt] <arquivo> ...\n",

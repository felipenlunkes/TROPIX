 ...
 *	for the tree codes used in the GNU C compiler		*
 *								*
 *	Versão	3.0.0, de 17.09.93				*
 *		3.0.0, de 17.09.93				*
 *								*
 *	Módulo: GCC						*
 *		Compilador "C"					*
...
/*
 ******	Argumentos da macro DEFTREECODE: ************************
 */

/*
 *	DEFTREECODE (enum tree_code, name, type, length)
 *
 *	1. code: No. de ordem do nó.
 *
 *	2. name: Names of tree components. Used for printing out
 *		 the tree and error messages.
 *
 *	3. type: Indexed by enum tree_code, contains a character which is
 *
 *		"<" for a comparison expression,
 *		"1", for a unary arithmetic expression,
 *		"2" for a binary arithmetic expression,
 ...
 *		"t" for a type object,
 *		"s" for a statement,
 *		"x" for anything else (TREE_LIST, IDENTIFIER, etc).
 *
 *	4. size: For "r", "e" and "x" nodes, the number of argument slots
 *		 to allocate. This determines the size of the tree node object.
 *		 (Number of expression operands beyond the fixed part of the
 *		 node structure. Not used for types or decls.)
 */

/*
 ******	Lista dos tipos dos nós: ********************************
 */
...
/*
 *	CHAR in Pascal; not used in C. No special fields needed.
 */ 
 ...
 *	Evaluates the expression STMT_BODY, then returns from the current
 *	function. Presumably STMT_BODY is an assignment that stores into the
...
/*
 *	Postfix & in C++.  Value is a reference to the object which is the
 *	operand. Operand type is preserved, but we know that it is no longer
 *	an lvalue.
 */

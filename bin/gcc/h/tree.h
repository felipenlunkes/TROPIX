 ...
 *
 *	See the accessor macros, defined below,
 *	for documentation of the fields.
 */
struct tree_common
{
	int		uid;
 ...
 *	 Codes are defined in tree.def.v.
 */
#define TREE_CODE(NODE) ((enum tree_code) (NODE)->common.code)
#define TREE_SET_CODE(NODE, VALUE) ((NODE)->common.code = (int) (VALUE))

/*
 *	In all nodes that are expressions, this is the data type
 *	of the expression. In POINTER_TYPE nodes, this is the type
 ...
 *	Types are chained together to record them for being output
 *	to the debugger (see the function `chain_type').
...
/*
 *	In a VAR_DECL, nonzero means allocate static storage but not common.
 *	In a FUNCTION_DECL, currently nonzero if function has been defined.
 */
#define TREE_ENTRY(NODE) ((NODE)->common.entry_attr)
 ...
 *	May also appear in an arithmetic expression,
 *	an ADDR_EXPR or a CONSTRUCTOR if the value is constant.
 */
#define TREE_LITERAL(NODE) ((NODE)->common.literal_attr)

/*
 *	Nonzero in a ..._DECL means this variable is ref'd from a
 *	nested function. Cannot happen in C because it does not allow
 *	nested functions, as of now.
...
/*
 ****** Additional fields and accessors *************************
 */

/*
 *	Define additional fields and accessors
 *	for nodes representing constants.
 */
...
{
	char		common[sizeof (struct tree_common)];
	struct rtx_def	*rtl;	/* acts as link to register transfer */
				/* language (rtl) info */
...
/*
 ******	For nodes representing data types ***********************
 */
...
/*
 *	See tree.def.v for documentation of the use of these fields.
...
struct tree_type
{
	...
	/*
	 *	Points to a structure whose details depend
...
struct tree_if_stmt
{
	...
	char		*filename;
	int		linenum;
	union tree_node	*cond;
	union tree_node	*thenpart;
	union tree_node	*elsepart;
};
...
/*
 *	integer_zerop (tree x) is nonzero if X
 *	is an integer constant of value 0
 */
extern int	integer_zerop ();

/*
 *	integer_onep (tree x) is nonzero if X
 *	is an integer constant of value 1
 */

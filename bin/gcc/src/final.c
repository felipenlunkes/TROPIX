 ...
 *			final.c					*
 *								*
 *	Convert RTL to assembler code and output it		*
 *								*
...
#if (0)	/*************************************/
#define	isinf(value)	0	/* PROVISÓRIO */
#endif	/*************************************/
  ...
  switch (GET_CODE (insn))
	  ...
	  && GET_CODE (NEXT_INSN (insn)) == JUMP_INSN)
	{
	  ...
	  if (GET_CODE (nextbody) == ADDR_VEC
	    ...
	    {
	      ASM_OUTPUT_CASE_LABEL (file, LPREFIX, CODE_LABEL_NUMBER (insn),
				     NEXT_INSN (insn));
	      break;
	    }
	}
      ...
      ASM_OUTPUT_INTERNAL_LABEL (file, LPREFIX, CODE_LABEL_NUMBER (insn));
      break;
     ...
     rtx x;
{
  ...
  if (GET_CODE (x) == LABEL_REF)
    ASM_GENERATE_INTERNAL_LABEL (buf, LPREFIX, CODE_LABEL_NUMBER (XEXP (x, 0)));
  else if (GET_CODE (x) == CODE_LABEL)
    ASM_GENERATE_INTERNAL_LABEL (buf, LPREFIX, CODE_LABEL_NUMBER (x));
  else

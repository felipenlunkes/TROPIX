 ...
 *			varasm.c				*
 *								*
 *	Output variables, constants and external declarations	*
 *								*
 ...
 ****************************************************************
 */
/*	Assemble a string constant with the specified
 *	C string as contents
 */
void
assemble_string (char *str, int size)
{
	register int	c, sz;
	register char	*bp = str, *ep;
	 ...
	 */
	size--;		/* Não precisamos do '\0' */

	if (size == 0)
		{ fprintf (fp, "	.isoz	\"\"\n"); return; }

	/*
	 *	Escreve em várias linhas de até MAXLEN bytes
	 */
	while (size > 0)
	{
		if (size <= MAXLEN)
			{ sz = size;	fprintf (fp, "	.isoz	\""); }
		else
			{ sz = MAXLEN;	fprintf (fp, "	.iso	\""); }

		for (ep = bp + sz; bp < ep; bp++)
		{
			if   ((c = *bp) == '\"' || c == '\\')
			{
				putc ('\\', fp);
 ...
 */
void
assemble_string (unsigned char *p, int size)
{
	register int	i;
	int		excess = 0;
	int		pos = 0;
	...
	while (pos < size)
	{
		...
		for (i = 0; i < thissize; i++)
		{
			...
			else
			{
				 ...
				 *	follows, terminate one string constant
				 *	and start another. The Vax assembler
				 *	fails to stop reading the escape after
				 *	three digits, so this is the only way
				 *	we can get it to parse the data
				 *	properly
				 */
				if
				(	i < thissize - 1 &&
		...
		int at_end;
{
	...
	if (DECL_INITIAL (decl) == 0 || DECL_INITIAL (decl) == error_mark_node)
	{
		...
		/*
		 *	Don't allocate zero bytes of common, since
		 *	that means "undefined external" in the linker
		 */
		if (size == 0)
			rounded = 1;
		 ...
		 *	so that each uninitialized object starts on such a
		 *	boundary
		 */
		...
		if   (TREE_ENTRY (decl))
			ASM_OUTPUT_ENTRY (asm_out_file, name, size, rounded);
		elif (TREE_PUBLIC (decl))
...
#ifdef SELECT_VAR_SECTION
	SELECT_VAR_SECTION (decl);
#else
...
#ifdef SELECT_LABEL_SECTION
	SELECT_LABEL_SECTION (exp);
#else

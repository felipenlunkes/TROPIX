 ...
 *			global-alloc.c				*
 *								*
 *	Allocate registers for pseudo-registers that		*
 *	span basic blocks					*
 *								*
...
#define	NEW
	...
	register int	new_pri1 =
		(floor_log2 (reg_n_refs[r1]) * reg_n_refs[r1] * 100000) /
			(reg_live_length[r1] * allocno_size[*v1]);

	register int	new_pri2 =
		(floor_log2 (reg_n_refs[r2]) * reg_n_refs[r2] * 100000) /
			(reg_live_length[r2] * allocno_size[*v2]);

#ifdef	NEW
	if (new_pri2 > new_pri1)
		return (+1);

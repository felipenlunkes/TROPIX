...
#define ASM_OUTPUT_DOUBLE(FILE,VALUE)  \
  fprintf (FILE, "\t.double\t%.15E\n", (VALUE))

/* This is how to output an assembler line defining a `float' constant.  */

#define ASM_OUTPUT_FLOAT(FILE,VALUE)  \
  fprintf (FILE, "\t.float\t%.15E\n", (VALUE))
...
/*
 *	Coloca a tabela do "switch" no ".const"
 */
#define ASM_OUTPUT_CASE_LABEL(FILE, PREFIX, LABEL_NUMBER, INSN)		\
									\
	...
	const_section ();						\
									\
	ASM_OUTPUT_INTERNAL_LABEL (FILE, PREFIX, LABEL_NUMBER);		\
									\
	vlen = XVECLEN (nextbody, 0);					\
									\
	for (idx = 0; idx < vlen; idx++)				\
	{								\
		ASM_OUTPUT_ADDR_VEC_ELT					\
		(	FILE,						\
			CODE_LABEL_NUMBER (XEXP (XVECEXP (nextbody, 0, idx), 0))\
		);							\
	}								\
									\
	text_section ();						\

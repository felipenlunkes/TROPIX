     ...
     rtx insn;
{
  ...
  if (top_dead_p (insn))
	...
	fputc (*rev++, asm_out_file);
      ...
      fprintf (asm_out_file, AS2 (p, f0, f1));
#if (0)	/*************************************/
	...
	fputc (*op++, asm_out_file);
      ...
      fprintf (asm_out_file ,AS2 ( , f1, f0));
#if (0)	/*************************************/
 ...
 */
void
function_prologue (register FILE *file, int size)
{
	register int	regno;
	int		nregs, limit;
	rtx		xops[4];
	...
	/*
	 *	Implicitamente assume xops[0] == sp e xops[1] == fp
	 */
	if (frame_pointer_needed)
	{
		output_asm_insn ("link	%2", xops);
	...
	/*
	 *	Note If use enter it is NOT reversed args. This one is not
	 *	reversed from intel!! I think enter is slower. Also sdb
	 *	doesn't like it. But if you want it the code is:
	 *		 {
	 *			 xops[3] = const0_rtx;
 ...
 */
void
function_epilogue (register FILE *file, int size)
{
	register int	regno;
 ...
 */
void
print_operand (register FILE *file, register rtx x, int code)
{
	if (code)
	{
		switch (code)
	...
	elif (GET_CODE (x) == CONST_DOUBLE && GET_MODE (x) == DFmode)
	{
		union { double d; int i[2]; } u;
		u.i[0] = CONST_DOUBLE_LOW (x);
		u.i[1] = CONST_DOUBLE_HIGH (x);
	...
	else 
	{
		if (code != 'c')
		{
			if (GET_CODE (x) == CONST_INT)
				PRINT_IMMED_PREFIX (file);
			elif (GET_CODE (x) == CONST || GET_CODE (x) == SYMBOL_REF || GET_CODE (x) == LABEL_REF)
...
print_operand_address (register FILE *file, register rtx addr)
{
	...
	rtx			offset;
	static const char	scale_vec[] = "?bw?l???q";
	int			scale = 1;
	...
	switch (GET_CODE (addr))
	{
		...
		elif (reg1 != 0 && REGNO_OK_FOR_BASE_P (REGNO (reg1)))
		{
			breg = reg1;
			ireg = reg2;
		}
		...
		if (ireg != 0)
		{
			...
			if (scale != 1)
				fprintf (file, "@%c", scale_vec[scale]);
		}
...
#if (0)	/*************************************/
	  PRINT_B_I_S (breg, ireg, scale_char, file);
#endif	/*************************************/
		goto print_bis;
...
#if (0)	/*************************************/
	PRINT_B_I_S ((rtx)0, ireg, scale_char, file);
#endif	/*************************************/

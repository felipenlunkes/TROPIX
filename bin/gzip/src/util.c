 ...
 *			util.c					*
 *								*
 *	utility functions for gzip support			*
 *								*
 ...
 ****************************************************************
 */
extern const ulong crc_32_tab[];	/* crc table, defined below */

/*
 ******	Protótipos de funções ***********************************
 */
int	getpercent (long, long);
char	*editpercent (int quoc);
 ...
 */
ulong
updcrc (register char *s, register unsigned n)
{
	register ulong	c;
...
/*
 ****************************************************************
 *	Escreve um bloco do arquivo ZIP				*
 ****************************************************************
 */
void
write_zip (int c, register ZIPFILE *zp)
{
	int		count;

	count = zp->z_ptr - zp->z_base;

	write_buf (ofd, zp->z_base, zp->z_ptr - zp->z_base);
...
/*
 ****************************************************************
 *	Does the same as write, but also handles partial writes	*
 ****************************************************************
 */
void
write_buf (int fd, void *buf, unsigned cnt)
{
	unsigned	  n;
 ...
 */
void
gzip_error (const char *m)
{
	fprintf (stderr, "\n%s: %s: %s\n", pgname, ifname, m);
	abort_gzip (0, NOVOID, NOVOID);
...
read_error (void)
{
	...
	if (errno != 0)
		...
		fprintf (stderr, "%s: Final inesperado do arquivo\n", ifname);
 ...
 */
void
display_ratio (long num, long den)
{
#if (0)	/*************************************/
	...
	fprintf (stderr, "%2ld.%ld%%", ratio / 10, ratio % 10);
#endif	/*************************************/

	fprintf (stderr, " (%s %%)", editpercent (getpercent (num, den)));
